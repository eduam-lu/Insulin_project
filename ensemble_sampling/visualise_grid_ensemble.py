...
(default `ensemble.out`), produces a `viz/` subdirectory with:
  ...
  PyMOL scripts (open with `@file.pml`):
    view.pml, reachable.pml                   trajectories, loops coloured by chainbreak
    placements.pml, reachable_placements.pml  per-cell, loops coloured by closure fraction
...
def loop_and_placed_residues(cfg: dict) -> Tuple[List[int], List[int]]:
    """Return (loop_residues, placed_residues).

    loop_residues  : residues from flexible segments (the linker being sampled)
    placed_residues: residues from rigid segments WITH a `dof` block (the mobile
                     domain whose position is being grid-swept)
    """
    loop: List[int] = []
    placed: List[int] = []
    for seg in cfg["segments"]:
        residues = list(range(seg["start"], seg["stop"] + 1))
        if seg["kind"] == "flexible":
            loop.extend(residues)
        elif seg["kind"] == "rigid" and seg.get("dof"):
            placed.extend(residues)
    return sorted(loop), sorted(placed)


def _resi_str(residues: List[int]) -> str:
    """Compact PyMOL `resi` selection string, e.g. '100-120+150+200-250'.

    Consecutive runs are collapsed to ranges; individual residues are left as
    single numbers.  Returns 'none' for an empty list so it can be used
    directly inside a PyMOL selection without breaking the parser.
    """
    if not residues:
        return "none"
    res = sorted(residues)
...
def write_multi_model_pdb(path: Path,
                          ...
                          residues: List[int], chain: str) -> List[float]:
    """Write one MODEL block per (coords, bfactor) entry.

    Returns the bfactor values of the MODEL blocks actually written, in file
    order (entries with no atoms for the requested residues are skipped), so
    the PML can colour each split object without relying on PyMOL's B column.
    """
    n_written = 0
    written: List[float] = []
    with open(path, "w") as fh:
...
def write_ensemble_pml(path: Path, pdb_name: str, values: List[float],
                       loop_res: List[int], placed_res: List[int],
                       title: str, prefix: str, quantity: str) -> None:
    """PML showing every sampled loop, each coloured by its own value.

    PDBs are CA-only and, in a multi-state object, atom properties (B,
    colour) are shared across states. So each MODEL is split into its own
    object and the per-model value is written as a plain `alter` line.

    Deliberately NO `python ... python end` block: an earlier version used
    the invalid terminator `end python`, which leaves PyMOL buffering every
    later command (in that file and the rest of the session) without
    running it. Plain PML plus single-line `print(...)` checkpoints only.

    ref           : grey cartoon
    placed domain : grey cartoon
    sampled loop  : ribbon (CA trace), blue_white_red
      chainbreak       -> log10, blue = closed, red = broken
      closure_fraction -> 0-1,   blue = 0, red = 1
    """
    placed_sel = f"resi {_resi_str(placed_res)}" if placed_res else "none"
    loop_sel = (f"resi {_resi_str(loop_res)}" if loop_res
                else f"not ({placed_sel})")
    join_sel = (f"({loop_sel}) or resi {min(placed_res)}"
                if placed_res else loop_sel)

    if quantity == "chainbreak":
        vals = [math.log10(max(v, 1e-3)) for v in values]
        lo, hi = (min(vals), max(vals)) if vals else (0.0, 1.0)
        if hi <= lo:
            hi = lo + 1.0
        legend = (f"log10(chainbreak), blue = {10**lo:.3g}, "
                  f"red = {10**hi:.3g}")
    else:
        vals = [float(v) for v in values]
        lo, hi = 0.0, 1.0
        legend = "closure fraction, blue = 0, white = 0.5, red = 1"

    # split_states names objects <prefix>_0001, _0002, ... in MODEL order.
    alters = "\n".join(f"alter {prefix}_{i:04d}, b={v:.5f}"
                       for i, v in enumerate(vals, start=1))

    body = f"""\
# Auto-generated by visualise_grid_ensemble.py
# {title}
# colour: {legend}

bg_color white
load ref.pdb, ref
load {pdb_name}, {prefix}_all
split_states {prefix}_all, prefix={prefix}_
delete {prefix}_all
group {prefix}, {prefix}_*
print("[pml] loaded %d objects" % len(cmd.get_names()))

hide everything
show cartoon, ref
color grey70, ref

# per-object value ({len(vals)} objects)
{alters}

# placed domain
show cartoon, {prefix} and {placed_sel}
color grey70, {prefix} and {placed_sel}

# sampled loops
show ribbon, {prefix} and ({join_sel})
spectrum b, blue_white_red, {prefix} and ({loop_sel}), minimum={lo:.5f}, maximum={hi:.5f}
color grey70, {prefix} and {placed_sel}
print("[pml] cartoon atoms %d, ribbon atoms %d" % (cmd.count_atoms("rep cartoon"), cmd.count_atoms("rep ribbon")))
...
def _grid_scatter(ax, x_vals, y_vals, Z_nx_ny,
    ...
    **kwargs          : forwarded to the coloured scatter call (cmap, vmin, vmax, …)
    ...
    # Dashed lines connecting the grid nodes (drawn first, behind the dots)
    line_kw = dict(color='#999999', linestyle='--', linewidth=0.8,
                   alpha=0.7, zorder=0)
    for y in y_vals:
        ax.plot(x_vals, np.full(len(x_vals), y), **line_kw)
    for x in x_vals:
        ax.plot(np.full(len(y_vals), x), y_vals, **line_kw)

    # Remove the surrounding box; keep only tick labels at exact grid positions
    ...
    # Coloured circles for valid cells (no edge so colours read cleanly)
    valid = ~nan_mask
    sc = None
    if np.any(valid):
...
def plot_gap_vs_closure(path: Path, cells: Dict[int, dict],
    ...
    if not viable:
        ...
        sc = ax.scatter(gaps, fracs, c=fracs, cmap="jet", vmin=0, vmax=1,
...
def plot_summary(path: Path, grid: dict, cells: Dict[int, dict],
    ...
    if viable:
        gaps = np.array([c["loop_gap"] for c in viable])
        fracs = np.array([c["closure_fraction"] for c in viable])
        ax.scatter(gaps, fracs, c=fracs, cmap="jet", vmin=0, vmax=1,
...
def main():
    ...
    loop_res, placed_res = loop_and_placed_residues(cfg)
    ref_coords = pose_ca_by_pdb_num(ref_pose)
    print(f"Loop      : {len(loop_res)} residues  "
          f"({loop_res[0]}–{loop_res[-1]})" if loop_res else "Loop      : none")
    print(f"Placed    : {len(placed_res)} residues "
          f"({placed_res[0]}–{placed_res[-1]})" if placed_res else "Placed    : none")
    ...
    v_all = write_multi_model_pdb(out_dir / "ensemble_all.pdb",
                                  _row_entries(sample_idx),
                                  mobile_res, chain)
    v_reach = write_multi_model_pdb(out_dir / "ensemble_reachable.pdb",
                                    _row_entries(reach_capped),
                                    mobile_res, chain)
    print(f"Wrote     : ensemble_all.pdb ({len(v_all)} MODELs), "
          f"ensemble_reachable.pdb ({len(v_reach)} MODELs)")
    ...
    v_p_all = write_multi_model_pdb(out_dir / "placements_all.pdb",
                                    placement_entries, mobile_res, chain)
    v_p_reach = write_multi_model_pdb(out_dir / "placements_reachable.pdb",
                                      placement_entries_reachable,
                                      mobile_res, chain)
    print(f"Wrote     : placements_all.pdb ({len(v_p_all)} MODELs), "
          f"placements_reachable.pdb ({len(v_p_reach)} MODELs)")

    # ---- PMLs ----
    write_ensemble_pml(out_dir / "view.pml", "ensemble_all.pdb", v_all,
                       loop_res, placed_res,
                       "All trajectories, loops coloured by chainbreak",
                       prefix="traj", quantity="chainbreak")
    write_ensemble_pml(out_dir / "reachable.pml", "ensemble_reachable.pdb",
                       v_reach, loop_res, placed_res,
                       f"Reachable trajectories (chainbreak < "
                       f"{args.close_threshold:g}), loops coloured by chainbreak",
                       prefix="reach", quantity="chainbreak")
    write_ensemble_pml(out_dir / "placements.pml", "placements_all.pdb",
                       v_p_all, loop_res, placed_res,
                       "Best model per viable cell, loops coloured by "
                       "closure fraction",
                       prefix="cell", quantity="closure_fraction")
    write_ensemble_pml(out_dir / "reachable_placements.pml",
                       "placements_reachable.pdb", v_p_reach,
                       loop_res, placed_res,
                       f"Best model per reachable cell (closure >= "
                       f"{args.reach_threshold:g}), loops coloured by "
                       f"closure fraction",
                       prefix="rcell", quantity="closure_fraction")
