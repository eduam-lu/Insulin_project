...
3. Sample rigid-body placements within the declared limits.
...
@dataclass
class DOF:
    """Maximum displacements for one rigid segment. An absent axis means fixed."""
    tx: float = 0.0
    ty: float = 0.0
    tz: float = 0.0
    tilt: float = 0.0    # degrees, away from the segment's own long axis
    spin: float = 0.0    # degrees, about the segment's own long axis

    @classmethod
    def from_dict(cls, d: Optional[dict]) -> "DOF":
        if not d:
            return cls()
        bad = set(d) - {"translate", "tilt", "spin"}
        if bad:
            raise ValueError(f"Unknown dof keys: {sorted(bad)}")
        trans = d.get("translate") or {}
        bad = set(trans) - {"x", "y", "z"}
        if bad:
            raise ValueError(f"Unknown translate axes: {sorted(bad)}")
        return cls(
            tx=float(trans.get("x", 0.0)),
            ty=float(trans.get("y", 0.0)),
            tz=float(trans.get("z", 0.0)),
...
def sample_placement(pose, cfg: Construct, jumps: Dict[str, int],
                     rng: random.Random) -> Dict[str, Dict[str, float]]:
    """
    Generate one starting configuration, in place, by perturbing each mobile
    rigid segment's jump within the limits declared in its dof block.

    Order is spin -> tilt -> translate. Spin is about the segment's own long
    axis; for a lone helix that looks like a no-op, but it moves the terminal
    attachment points azimuthally, which changes what the flanking loops have
    to bridge.

    Returns the drawn values so they can be logged alongside each model.
    """
    block_of = {r.name: (lo, hi) for r, lo, hi in cfg.blocks()}
    drawn: Dict[str, Dict[str, float]] = {}

    for seg in cfg.rigid:
        if seg.name not in jumps or not seg.dof.is_mobile:
            continue          # the root, or a rigid body declared with no dof
        ...
        params = {
            ...
            # Uniform in [0, tilt_max]. Not an isotropic draw on the sphere
            # (which would weight by sin); this deliberately over-samples small
            # tilts, since upright is the reference state.
        ...
        if params["tilt"]:
            tilt_axis = perpendicular_to(own_axis, params["tilt_azimuth"])
            rotate_jump(pose, jid, tilt_axis, params["tilt"], centre)

        dist = math.sqrt(params["dx"] ** 2 + params["dy"] ** 2 + params["dz"] ** 2)
        if dist > 1e-9:
            translate_jump(pose, jid,
                           (params["dx"] / dist, params["dy"] / dist, params["dz"] / dist),
                           dist)

        drawn[seg.name] = params
...
def parse_args():
    ...
    p.add_argument("--n-traj", type=int, default=10,
                   help="MC trajectories per placement")
    ...
    p.add_argument("--max-attempts-factor", type=int, default=20,
                   help="Give up after n-placements * this many rejected draws")
...
def main():
    ...
    kept_placements = 0
    attempts = 0
    max_attempts = max(args.n_placements * args.max_attempts_factor, args.n_placements)

    while kept_placements < args.n_placements and attempts < max_attempts:
        attempts += 1

        placed = reference.clone()
        drawn = sample_placement(placed, cfg, jumps, rng)
        ...
        for t_idx in range(args.n_traj):
            ...
            for s_idx, pose in enumerate(mc_torsion_sampling(
                ...
                for name, params in drawn.items():
                    rec.update({f"{name}_{k}": round(v, 3) for k, v in params.items()})
                records.append(rec)
        ...
        kept_placements += 1
        print(f"[placement {p_idx}] {args.n_traj} trajectories, "
              f"{n_from_placement} structures "
              f"({kept_placements}/{args.n_placements} placements)")
    ...
    with open(outdir / "metrics.json", "w") as fh:
        json.dump({"config": args.config, "pdb": args.pdb, "seed": args.seed,
                   ...
                   "n_traj": args.n_traj,
                   "placements": kept_placements, "placement_attempts": attempts,
                   "models": records}, fh, indent=2)
    ...
    if kept_placements < args.n_placements:
